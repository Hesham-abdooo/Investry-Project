...
export default function FounderProfile() {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://investry.runasp.net/api/Accounts/upload-profile-picture",
        {
          method: "POST",
          headers: {
      ...
      setProfileImage(null); // ارجع للصورة القديمة لو فشل
    } finally {
  ...
  return (
    <>
      <div className="max-w-150 mx-auto px-6 pt-8 pb-16 sm:px-4 sm:pt-5 sm:pb-10">
        {/* Back to Home */}
        <button
          type="button"
        ...
        {/* Page Title */}
        <h1 className="text-[26px] font-extrabold text-slate-900 mb-6 tracking-tight sm:text-[22px] sm:mb-4.5">
          Edit Profile
        </h1>
        ...
        <div className="bg-white border border-[#e8ebf0] rounded-2xl shadow-[0_1px_3px_rgba(15,23,42,0.04),0_4px_12px_rgba(15,23,42,0.03)]">
          {/* ─── Profile Picture ─── */}
          <div className="px-8 py-7 sm:px-5 sm:py-5.5">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                type="button"
                className="px-4.5 py-2 border border-gray-300 rounded-lg bg-white text-sm font-medium text-slate-800 cursor-pointer transition-all duration-200 hover:bg-slate-50 hover:border-slate-400 hover:text-slate-900"
          ...
          <div className="px-8 py-7 sm:px-5 sm:py-5.5">
            ...
            {kycVerified ? (
              /* ── Verified ── */
              <div className="flex items-center justify-between bg-slate-50 border border-[#e8ebf0] rounded-[10px] px-4.5 py-3.5 transition-all duration-300 sm:flex-col sm:items-start sm:gap-2.5">
                <span className="text-sm font-medium text-slate-700">
                  Identity Verification
                </span>
                <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600 tracking-wide">
                  <span className="w-1.75 h-1.75 rounded-full bg-emerald-500" />
                  ACTIVE
                </span>
              </div>
            ) : (
              /* ── Not Verified ── */
              <div className="bg-slate-50 border border-[#e8ebf0] rounded-[10px] px-4.5 py-4 transition-all duration-300">
                <div className="flex items-center justify-between mb-3 sm:flex-col sm:items-start sm:gap-2">
                  ...
                  <span className="inline-flex items-center gap-1.5 text-xs font-bold text-amber-600 tracking-wide">
                    <span className="w-1.75 h-1.75 rounded-full bg-amber-500" />
                    PENDING
                  </span>
                </div>
                <p className="text-[13px] text-slate-500 leading-relaxed mb-4">
                  Complete your KYC verification to unlock all platform
                  features. This process is handled by our secure verification
                  partner.
                </p>
                <button
                  type="button"
